import { getRenderConf, getPoints, getPoint, getAreaProps } from "./coordinate-utils";
...
function initListener(canvas) {
    ...
    eventArr.forEach((e) => win.addEventListener(e, updateHandler.bind(this), false));
...
function drawArea(arr, areaProps) {
    ...
    let points = getPoints(arr, conf.minX, conf.minY, conf.offsetX, conf.offsetY, conf.areaScale, this.height);
...
function convert(coordinate) {
    let conf = this.renderConf;
    return getPoint(coordinate, conf.minX, conf.minY, conf.offsetX, conf.offsetY, conf.areaScale, this.height);
}
...
export function initMap(el, width, height) {
    const target = typeof this.el === "string" ? doc.querySelector(this.el) : this.el;
...
export function updateMap() {
    ...
    if (!mouseOnMap && this.hoverName !== null) {
        this.callLayer("onAreaLeave", getAreaProps(this.hoverName, this.geoJSON));
        this.hoverName = null;
    }
